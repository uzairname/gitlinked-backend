...
```bash
sam-pytorch$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sam-pytorch$ sam local start-api
sam-pytorch$ curl http://localhost:3000/classify_digit
```
...
```bash
sam-pytorch$ sam logs -n InferenceFunction --stack-name sam-pytorch --tail
```
...
```bash
aws cloudformation delete-stack --stack-name sam-pytorch
```
